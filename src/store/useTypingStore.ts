        ...
        if (user) {
          try {
            await supabase.from('test_results').insert({
              ...
              wpm: Math.round(finalResult.wpm),
              raw_wpm: Math.round(finalResult.rawWpm),
              accuracy: Math.round(finalResult.accuracy),
              consistency: Math.round(finalResult.consistency),
              mode: finalResult.mode,
              duration: Math.round(finalResult.duration),
        ...
        try {
          ...
          if (toSync.length > 0) {
            ...
            const insertData = toSync.map(item => ({
              ...
              raw_wpm: Math.round(item.rawWpm),
              accuracy: Math.round(item.accuracy),
              consistency: Math.round(item.consistency),
              mode: item.mode,
