...
export function ModeSelector() {
  ...
  return (
    <div className="flex flex-col items-center gap-4 animate-fade-in-down">
      {/* Mode Selector */}
      <div className="flex items-center gap-2 p-2 bg-panel/50 backdrop-blur-lg rounded-xl border border-border/40 shadow-lg">
        {modes.map((m) => (
          <button
            key={m.id}
      ...
      {/* Options Selector */}
      {testMode === "time" && (
        <div className="flex items-center gap-2 p-2 bg-panel/50 backdrop-blur-lg rounded-xl border border-border/40 shadow-lg animate-scale-in">
          {timeOptions.map((t) => (
            <button
              key={t}
              ...
              disabled={isRunning}
              className={cn(
                "px-4 py-2 rounded-lg text-sm font-medium transition-all duration-200 min-w-[56px]",
                "hover:bg-muted/30 disabled:opacity-50 disabled:cursor-not-allowed",
                duration === t
